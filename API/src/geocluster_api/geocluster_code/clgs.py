...
from .sbt.sbt_v27 import run_sbt as run_sbt_final
...
class Data:
        ...
        if sbt_version == 0:
            ...
            try:
                times, Tout, Pout = run_sbt_final(
                    ...
                    mdot=mdot,
                    Tinj=Tinj,
                    fluid=fluid,  ## Operations
                    DrillingDepth_L1=L1,
                    HorizontalExtent_L2=L2,  # BoreholeDiameter=D, ## Wellbore Geometry
                    Diameter1=Diameter1,
                    Diameter2=Diameter2,
        ...
        try:
            ...
            for i in range(len(points_to_read_around)):
                if i == 0 or i == var_index:
                    ...
                    points_to_read_around[i] = parameters_to_read_from_point[
                        point_index
                    ]
                    point_index += 1
    ...
    def interp_kW(
        ...
        mdot = point[0]
        Tinj = point[5]
        return interp_kW_standalone(
            mdot, Tinj, Tout, Pout, self.Pinj, self.CP_fluid, Tamb
        )
...
def interp_kW_standalone(mdot, Tinj, Tout, Pout, Pinj, CP_fluid, Tamb=300.0):
    enthalpy_out = CP.PropsSI("H", "T", Tout, "P", Pout, CP_fluid)
    enthalpy_in = CP.PropsSI("H", "T", Tinj, "P", Pinj, CP_fluid)
    entropy_out = CP.PropsSI("S", "T", Tout, "P", Pout, CP_fluid)
    entropy_in = CP.PropsSI("S", "T", Tinj, "P", Pinj, CP_fluid)

    kWe = (
        mdot * (enthalpy_out - enthalpy_in - Tamb * (entropy_out - entropy_in)) / 1000.0
    )
    kWt = mdot * (enthalpy_out - enthalpy_in) / 1000.0
