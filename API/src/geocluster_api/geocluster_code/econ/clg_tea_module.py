...
class TEA:
    def __init__(self, Fluid, End_use, Configuration, Flow_user, Hor_length_user, Depth_user, Gradient_user, 
    ...
    def extract_result_vars(self):
        all_vars = vars(self)
        vars_in_both = [
            "AveProductionTemperature",
            "AveProductionPressure",
            "TotalCAPEX",
            "CAPEX_Drilling",
            "CAPEX_Surface_Plant",
            "AverageOPEX_Plant",
            "AveInstHeatProduction",
            "FirstYearHeatProduction",
        ]
        end_use_1_vars = [
            "LCOH"
        ]
        end_use_2_vars = [
            "AveInstNetElectricityProduction",
            "FirstYearElectricityProduction",
            "LCOE"
        ]

        additional_vars = end_use_1_vars if self.End_use==1 else end_use_2_vars
        total_vars = vars_in_both + additional_vars
        ans = {key: all_vars[key] for key in total_vars}
        return ans